...
#### Slicing
You can slice an array using the `Array<start,end>` so to get the first 10 entries that would be `Array<0,9>` (Note: Arrays start from 0). Like with JavaScript you can just negative numbers so to instead get the last 10 items it would be `Array<-1,-10>`.
